...
Each book must have: `id`, `title`, `author`, `year`, `description`, `genre`, `isFavorite`. Without a valid `db.json`, the API on port 3000 will not return data and the book list will not load.
...
Both servers must be running at the same time – Angular fetches books from `http://localhost:3000/books`.
...
| `npm run json-server` | Start mock API with `db.json` (port 3000) |
